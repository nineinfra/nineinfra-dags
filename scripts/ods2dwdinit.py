def get_ods2dim_init_sqls(data_base, start_date):
    ...
    dwd_traffic_page_view_inc = f"\n\
        ...
        select base_province.province_id,\n\
               ...
               log.page_id,\n\
               log.source_type,\n\
               date_format(from_utc_timestamp(from_unixtime(log.ts / 1000), 'GMT+8'), 'yyyy-MM-dd')                                                    as date_id,\n\
               date_format(from_utc_timestamp(from_unixtime(log.ts / 1000), 'GMT+8'), 'yyyy-MM-dd HH:mm:ss')                                           as view_time,\n\
    ...
    dwd_traffic_start_inc = f"\n\
        ...
        select base_province.province_id,\n\
               ...
               log.open_ad_id,\n\
               date_format(from_utc_timestamp(from_unixtime(log.ts / 1000), 'GMT+8'), 'yyyy-MM-dd')          as date_id,\n\
               date_format(from_utc_timestamp(from_unixtime(log.ts / 1000), 'GMT+8'), 'yyyy-MM-dd HH:mm:ss') as start_time,\n\
    ...
    dwd_traffic_action_inc = f"\n\
        ...
        select base_province.province_id,\n\
               ...
               log.user_id,\n\
               log.version_code,\n\
               CAST(log.during_time AS BIGINT),\n\
               log.page_item,\n\
               log.page_item_type,\n\
               log.last_page_id,\n\
               log.page_id,\n\
               ...
               date_format(from_utc_timestamp(from_unixtime(log.ts / 1000), 'GMT+8'), 'yyyy-MM-dd')          as date_id,\n\
               date_format(from_utc_timestamp(from_unixtime(log.ts / 1000), 'GMT+8'), 'yyyy-MM-dd HH:mm:ss') as action_time\n\
        from\n\
        (\n\
            select common.ar          as area_code,\n\
                   common.ba          as brand,\n\
    ...
    dwd_traffic_display_inc = f"\n\
        ...
        select base_province.province_id,\n\
               ...
               log.source_type,\n\
               date_format(from_utc_timestamp(from_unixtime(log.ts / 1000), 'GMT+8'), 'yyyy-MM-dd')          as date_id,\n\
               date_format(from_utc_timestamp(from_unixtime(log.ts / 1000), 'GMT+8'), 'yyyy-MM-dd HH:mm:ss') as display_time,\n\
               log.display_type,\n\
               log.display_item,\n\
               log.display_item_type,\n\
               CAST(log.display_order AS BIGINT),\n\
               CAST(log.display_pos_id AS BIGINT)\n\
        from\n\
        (\n\
            select common.ar             as area_code,\n\
                   common.ba             as brand,\n\
    ...
    dwd_traffic_error_inc = f"\n\
        ...
        select base_province.province_id,\n\
               ...
               log.displays,\n\
               date_format(from_utc_timestamp(from_unixtime(log.ts / 1000), 'GMT+8'), 'yyyy-MM-dd')          as date_id,\n\
               date_format(from_utc_timestamp(from_unixtime(log.ts / 1000), 'GMT+8'), 'yyyy-MM-dd HH:mm:ss') as error_time,\n\
               log.error_code,\n\
               log.error_msg\n\
        from\n\
        (\n\
    ...
    dwd_user_login_inc = f"\n\
        ...
        select user_id,\n\
               date_format(from_utc_timestamp(from_unixtime(log.ts / 1000), 'GMT+8'), 'yyyy-MM-dd')          as date_id,\n\
               date_format(from_utc_timestamp(from_unixtime(log.ts / 1000), 'GMT+8'), 'yyyy-MM-dd HH:mm:ss') as login_time,\n\
               log.channel,\n\
               base_province.province_id,\n\
               log.version_code,\n\
               log.mid_id,\n\
    ...
    dwd_init_sqls = [dwd_trade_cart_add_inc, dwd_trade_order_detail_inc,
                     dwd_trade_cancel_detail_inc, dwd_trade_pay_detail_suc_inc,
                     dwd_trade_order_refund_inc, dwd_trade_refund_pay_suc_inc,
                     dwd_trade_cart_full, dwd_tool_coupon_get_inc,
                     dwd_tool_coupon_order_inc, dwd_tool_coupon_pay_inc,
                     dwd_interaction_favor_add_inc, dwd_interaction_comment_inc,
                     dwd_traffic_page_view_inc, dwd_traffic_start_inc,
                     dwd_traffic_action_inc, dwd_traffic_display_inc,
                     dwd_traffic_error_inc, dwd_user_register_inc, dwd_user_login_inc]
    return dwd_init_sqls
