...
generate_sql = PythonOperator(task_id="generate_sql_task",
                              python_callable=gensql.generate_ods2dwd_sql,
                              op_kwargs={'datahouse_dir': Variable.get("datahouse_dir"),
                                         'start_date': airflow.utils.dates.days_ago(0).date()},
                              provide_context=True,
                              trigger_rule='all_done',
                              dag=dag_instance)
load_data = HiveOperator(hql=f'sqls/ods2dwd.sql',
                         task_id="load_data_task",
                         # default name of the hive connect of the airflow in the nineinfra
